...
class SleepEventClassificationDataset(Dataset):
    ...
    def _try_get_item(self, idx):
        ...
        # Resample embeddings from 5-second to 30-second resolution (factor of 6)
        # by mean-pooling every 6 consecutive embedding vectors. This aligns the
        # embedding temporal resolution to the label temporal resolution (30-sec epochs).
        RESAMPLE_FACTOR = 6
        T = x_data.shape[1]
        T_trim = (T // RESAMPLE_FACTOR) * RESAMPLE_FACTOR
        x_data = x_data[:, :T_trim, :]
        x_data = x_data.reshape(x_data.shape[0], T_trim // RESAMPLE_FACTOR, RESAMPLE_FACTOR, x_data.shape[2]).mean(dim=2)
